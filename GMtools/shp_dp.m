...
%   P: (to use the forward mapping constraint) cell array {fwd,I} where fwd
%      is a function handle for a forward mapping with inputs indexed by I.
...
%   pmodes: Nx1 cell array containing the modes' density value.
%   lambda: 1x2 vector containing weights for fwd constraint and nodes'
%      height respectively. Default: [0 0].
%   Q: 1x2 cell array to indicate distance measure. Defaut: {'M',0}.
%      Q{1}: 'M' (theta-x), 'C' (theta); Q{2}: 0 (square root), 1 (square).
% Out:
%   path: NxD matrix of N row D-dim vectors, the nodes of the path, in order.
...
if ~exist('P','var') | isempty(P)
  ...
  fwd = P{1};                     % Function handle to forward mapping
  I = P{2}; J = setdiff(1:D,I);
end
...
% cp(i,:) is the path from layer 1 (start) to node i of layer n (current).
% Thus, at stage n cp is a c(n) x n matrix.
% cl(i) is the length of path cp(i,:). Thus, at stage n cl is a c(n)x1 vector.
...
for n=2:N
  ...
  % d(i,j) = distance from node i of layer n-1 to node j of layer n
  switch Q{1}
   case 'M', d = sqdist(V{n-1},V{n},w);
   otherwise, d = sqdist(V{n-1}(:,J),V{n}(:,J),w(J));
  end
  if Q{2}==0 d = sqrt(d); end;

  % Forward constraint
  if ~isempty(fwd)
    d0 = sqdist(fwd(V{n-1}(:,J)),V{n-1}(1,I),w(I));
    if Q{2}==0 d0 = sqrt(d0); end;
    cl = cl + lambda(1)*d0;
  end
  
  % Mode heights
  if ~isempty(pmodes)
    cl = cl - lambda(2)*pmodes{n-1};
  end
  ...
  % d: local path score matrix between frame n-1 and n. d(i,j) is the
  ...
  % cp(temp',:): select relevant rows (see picture).

  [cl,temp] = min(bsxfun(@plus,cl,d),[],1);		% [*] see above
  cl = cl'; cp = [cp(temp',:) (1:size(V{n},1))'];
